...
*
*
*  Modified: 2006 Daniel 'Necr0Potenc3' Cavalcanti
...
*   Greets to Michal Dvorak (Mikee) for finding a function typo.
*
*   Things to know before usage:
*   Blowfish is independant of the key.
*   Blowfish+Twofish encryption scheme has to be done by calling Blowfish first.
*   On decryption you must call Twofish first, then Blowfish.
*
\******************************************************************************/
...
static unsigned int s_box[4 * 256] =
{
...
/* OLD/BLOWFISH CRYPT FUNCTIONS */

/* these tables should be built only once to keep memory usage to a minimum */
static unsigned int p_table[CRYPT_GAMEKEY_COUNT][18]; /* permutation table */
static unsigned int s_table[CRYPT_GAMEKEY_COUNT][1024]; /* substitution table */
...
void BlowfishInit(BlowfishObj *Obj)
{
	if(tables_ready == 0)
	{
		BlowfishInitTables(Obj);
		tables_ready = 1;
	}

	Obj->table_index = CRYPT_GAMETABLE_START;
    memcpy(Obj->seed, g_seed_table[0][Obj->table_index][0], CRYPT_GAMESEED_LENGTH);
    Obj->stream_pos = 0;
    Obj->block_pos = 0;
...
{
	int key_index = 0, i = 0;
	unsigned char *pkey = NULL, *pkey_end = NULL;
	unsigned int mask = 0;
	unsigned int value[2] = { 0, 0 };

    for(key_index = 0; key_index < CRYPT_GAMEKEY_COUNT; key_index++)
    {
        int i=0;
        ...
        pkey_end = g_key_table[key_index + 1];
        ...
        for(i = 0; i < 18; i += 2)
        {
            BlowfishRawEncrypt(value, key_index);
...
{
	int len_remaining = 0, i = 0;
	unsigned int values[2];
	unsigned char c = 0;
	unsigned char *seed = NULL;

    while(Obj->stream_pos + len > CRYPT_GAMETABLE_TRIGGER)
    {
        len_remaining = CRYPT_GAMETABLE_TRIGGER - Obj->stream_pos;

        BlowfishEncrypt(Obj, in, out, len_remaining);

        Obj->table_index = (Obj->table_index + CRYPT_GAMETABLE_STEP) % CRYPT_GAMETABLE_MODULO;
        memcpy(Obj->seed, g_seed_table[1][Obj->table_index][0], CRYPT_GAMESEED_LENGTH);
        Obj->stream_pos = 0;
        Obj->block_pos = 0;
    ...
    for(i = 0; i < len; i++)
    {
        if(Obj->block_pos == 0)
        {
            ...
            BlowfishRawEncrypt(values, Obj->table_index);
        ...
        c = (*in++) ^ Obj->seed[Obj->block_pos];
        *out++ = c;

		Obj->seed[Obj->block_pos] = c;
        Obj->block_pos = (Obj->block_pos + 1) % 8;
    }
...
void BlowfishDecrypt(BlowfishObj *Obj, unsigned char *in, unsigned char *out, int len)
{
    ...
    while(Obj->stream_pos + len > CRYPT_GAMETABLE_TRIGGER)
    {
        ...
        BlowfishDecrypt(Obj, in, out, len_remaining);
...
/* NEW/TWOFISH CRYPT FUNCTIONS */
void TwofishInit(TwofishObj *Obj)
{
    unsigned char tmpBuff[0x100];
	int i = 0;

    /* memcpy(&m_IP,IP,4); */ /* has to be set by the user before calling TwofishInit() */
    memset(&Obj->ki, 0, sizeof(keyInstance));
    memset(&Obj->ci, 0, sizeof(cipherInstance));

	Obj->dwIndex = 0;
	Obj->tabEnable = 0;
	Obj->pos = 0;
	Obj->numRounds[0] = 0;
	Obj->numRounds[1] = Obj->numRounds[2] = Obj->numRounds[3] = 0x10;

    TwofishMakeKey(Obj, &Obj->ki , 1, 0x80, NULL); /* makeKey(&ki,DIR_DECRYPT,0x80,NULL); */
    TwofishCipherInit(&Obj->ci, 1, NULL); /* cipherInit(&ci,MODE_ECB,NULL); */

    Obj->ki.key32[0] = Obj->ki.key32[1] = Obj->ki.key32[2] =
	Obj->ki.key32[3] = Obj->IP;
    TwofishReKey(&Obj->ki);

    for(i = 0; i < 256; i++)
        Obj->subData3[i] = i;

    TwofishBlockEncrypt(&Obj->ci, &Obj->ki, Obj->subData3, 256*8, tmpBuff);
    memcpy(Obj->subData3, tmpBuff, 256);
...
void TwofishEncrypt(TwofishObj *Obj, unsigned char *in, unsigned char *out, int len)
{
    ...
    for(i = 0; i < len; i++)
    {
        if(Obj->pos == 0x100)
        {
	    	TwofishBlockEncrypt(&Obj->ci, &Obj->ki, Obj->subData3, 0x800, tmpBuff);
            memcpy(Obj->subData3, tmpBuff, 0x100);
            Obj->pos = 0;
        }

        out[i] = in[i] ^ Obj->subData3[Obj->pos++];
    }
...
unsigned int TwofishF32(unsigned int x, unsigned int *k32,int keyLen)
{
	...
	*((unsigned int *)b) = Bswap(x);
	...
	/*if(Obj->tabEnable)
	{
		Obj->tabUsed[_b(x,0)] |= 0x02;
		Obj->tabUsed[_b(x,1)] |= 0x01;
		Obj->tabUsed[_b(x,2)] |= 0x02;
		Obj->tabUsed[_b(x,3)] |= 0x01;

		Obj->tabUsed[b[0] ] |= 0x04;
		Obj->tabUsed[b[1] ] |= 0x08;
		Obj->tabUsed[b[2] ] |= 0x10;
		Obj->tabUsed[b[3] ] |= 0x20;
	}*/
...
void TwofishReKey(keyInstance *key)
{
	...
	for(i = 0; i < k64Cnt; i++)
	{
		...
		key->sboxKeys[k64Cnt-1-i] = TwofishRS_MDS_Encode(k32e[i],k32o[i]); 
	}

	for(i = 0; i < subkeyCnt/2; i++)	
	{
		A = TwofishF32(i*0x02020202u        ,k32e,keyLen);
		B = TwofishF32(i*0x02020202u+0x01010101u,k32o,keyLen);
		B = ROL(B,8);
		key->subKeys[2*i  ] = A+  B;
...
void TwofishMakeKey(TwofishObj *Obj, keyInstance *key, unsigned char direction, int keyLen, char *keyMaterial)
{
	...
	key->numRounds	= Obj->numRounds[(keyLen-1)/64];
...
void TwofishBlockEncrypt(cipherInstance *cipher, keyInstance *key, unsigned char *input, int inputLen, unsigned char *outBuffer)
{
	...
	if(cipher->mode == 3)
	{
		...
		{
			TwofishBlockEncrypt(cipher, key, cipher->IV, 128, (unsigned char *)x);
			bit = 0x80 >> (n & 7);
	...
	for(n = 0; n < inputLen; n+=128, input+=16, outBuffer+=16)
	{
		for(i = 0; i < 4; i++)
		{
		...
		for(r = 0; r < rounds; r++)
		{	
			t0	 = TwofishF32(    x[0]   ,key->sboxKeys,key->keyLen);
			t1	 = TwofishF32(ROL(x[1],8),key->sboxKeys,key->keyLen);
